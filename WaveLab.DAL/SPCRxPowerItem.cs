...
{
    public class SPCRxPowerItem : AdoDaoSupport, ISPCRxPowerItem
    {
        public IList<SPCRxPowerItemInfo> Query(Hashtable hashTable, string sortby, string orderby)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT distinct rx_power_item_pk, type,mode,ch,pw ");
            cmdText.Append(" FROM  spc_rx_power_item ");
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<SPCRxPowerItemInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                ...
                item.RxPowerItemPK = Convert.ToInt32(reader["Rx_Power_Item_PK"]);
        ...
        public bool CheckExists(SPCRxPowerItemInfo item)
        {
            bool retVal;

            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("SELECT COUNT(*) FROM spc_rx_power_item ");
        ...
        public void Save(SPCRxPowerItemInfo item)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" insert into spc_rx_power_item(type,mode,ch,pw,Sampling_Lower,Sampling_Upper,lsl,usl,LCL_X,UCL_X,LCL_R,UCL_R,enable,last_update_date,last_updated_by) values( ");
            cmdText.Append(" @type,@mode,@ch,@pw,@Sampling_Lower,@Sampling_Upper,@lsl,@usl,@LCL_X,@UCL_X,@LCL_R,@UCL_R,@enable,@last_update_date,@last_updated_by)");
        ...
        {
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            paras.Create().Name("Rx_Power_Item_PK").Type(DbType.Int32).Value(RxPowerItemPK);

            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("SELECT Rx_Power_Item_PK,Type,Mode,CH,PW,Sampling_Lower,Sampling_Upper,LSL,USL,LCL_X,UCL_X,LCL_R,UCL_R,Enable ");
            cmdText.Append("FROM SPC_Rx_Power_Item ");
            cmdText.Append("WHERE 1=1 ");
            cmdText.Append("AND Rx_Power_Item_PK=@Rx_Power_Item_PK ");


            return AdoTemplate.QueryForObjectDelegate<SPCRxPowerItemInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCRxPowerItemInfo entity = new SPCRxPowerItemInfo();
                entity.RxPowerItemPK = Convert.ToInt32(reader["Rx_Power_Item_PK"]);
        ...
        public void Update(SPCRxPowerItemInfo item)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("UPDATE spc_rx_power_item SET ");
            ...
            cmdText.Append("WHERE Rx_Power_Item_PK=@Rx_Power_Item_PK ");
            ...
            paras.Create().Name("last_update_date").Type(DbType.DateTime).Size(4).Value(item.LastUpdateDate);
            paras.Create().Name("last_updated_by").Type(DbType.String).Size(50).Value(item.LastUpdatedBy);
            paras.Create().Name("Rx_Power_Item_PK").Type(DbType.Int32).Value(item.RxPowerItemPK);
            AdoTemplate.ExecuteNonQuery(CommandType.Text, cmdText.ToString(), paras.GetParameters());
        }

        public void Delete(SPCRxPowerItemInfo item)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("DELETE SPC_RX_POWER_ITEM WHERE Rx_Power_Item_PK=@Rx_Power_Item_PK ");
        ...
        public IList<SPCRxPowerItemLogInfo> GetLogs(int RxPowerItemPK)
        {
            ...
            cmdText.Append("FROM SPC_Rx_Power_Item_Log ");
            cmdText.Append("WHERE 1=1 ");
            cmdText.Append("AND Rx_Power_Item_PK=@Rx_Power_Item_PK ");
            cmdText.Append("ORDER BY Last_Update_Date Desc ");

            return AdoTemplate.QueryWithRowMapperDelegate<SPCRxPowerItemLogInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCRxPowerItemLogInfo entity = new SPCRxPowerItemLogInfo();
