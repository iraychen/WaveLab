...
{
    public class SPCSDPartRxIFGainXMR : AdoDaoSupport, ISPCSDPartRxIFGainXMR
    {
        public int QueryHistory(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT DISTINCT COUNT(*) ");
            cmdText.Append(" FROM SPC_SDPart_RxIF_Gain_XMR ");
        ...
        public IList<SPCSDPartRxIFGainXMRInfo> QueryHistory(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            ...
            cmdText.Append("XMR_PK,Station_No,Tx_Index,Serial_No,Date_From,Date_To ");
            cmdText.Append("FROM SPC_SDPart_RxIF_Gain_XMR ");
            cmdText.Append("WHERE 1=1 ");
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxIFGainXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxIFGainXMRInfo item = new SPCSDPartRxIFGainXMRInfo();
        ...
        public SPCSDPartRxIFGainXMRInfo Get(int XMRPK)
        {
            ...
            DetailCmdText.Append("FROM SPC_SDPart_RxIF_Gain_Detail ");
            DetailCmdText.Append("WHERE XMR_PK=@XMR_PK ");
            DetailCmdText.Append("ORDER BY No_Of_Times ASC ");

            IList<SPCSDPartRxIFGainDetail> DetailItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxIFGainDetail>(CommandType.Text, DetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxIFGainDetail item = new SPCSDPartRxIFGainDetail();
            ...
            ExceptionCmdText.Append("FROM SPC_SDPart_RxIF_Gain_Exception ");
            ExceptionCmdText.Append("WHERE XMR_PK=@XMR_PK ");

            IList<SPCSDPartRxIFGainException> ExceptionItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxIFGainException>(CommandType.Text, ExceptionCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxIFGainException item = new SPCSDPartRxIFGainException();
                item.NoOfTimes = Convert.ToInt32(reader["No_Of_Times"]);
                item.TestingDate = Convert.ToDateTime(reader["Testing_Date"]);
            ...
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("SELECT XMR_PK,Station_No,Tx_Index,Serial_No,Date_From,Date_To,X,R,CL_X,UCL_X,LCL_X,CL_R,UCL_R,LCL_R ");
            cmdText.Append("FROM SPC_SDPart_RxIF_Gain_XMR ");
            cmdText.Append("WHERE XMR_PK=@XMR_PK ");

            return AdoTemplate.QueryForObjectDelegate<SPCSDPartRxIFGainXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxIFGainXMRInfo entity = new SPCSDPartRxIFGainXMRInfo();
        ...
        public void SaveException(int XMRPK, IList<SPCSDPartRxIFGainException> ExceptionItems)
        {
            StringBuilder cmdText = new StringBuilder();
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            ...
            {
                cmdText.Append(" INSERT INTO SPC_SDPart_RxIF_Gain_Exception");
