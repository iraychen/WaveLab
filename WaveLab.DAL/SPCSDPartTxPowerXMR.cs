...
{
    public class SPCSDPartTxPowerXMR : AdoDaoSupport, ISPCSDPartTxPowerXMR
    {
        public int QueryHistory(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT DISTINCT COUNT(*) ");
            cmdText.Append(" FROM SPC_SDPart_Tx_Power_XMR a");
        ...
        public IList<SPCSDPartTxPowerXMRInfo> QueryHistory(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("SELECT * FROM ( ");
            cmdText.Append("SELECT rowindex = row_number() over (order by " + sortBy + " " + orderBy + " ) , ");
            cmdText.Append("a.XMR_PK,a.Station_No,a.Divide,a.CH_No,a.Mode,a.CH,a.PW,a.Serial_No,a.Date_From,a.Date_To ");
            cmdText.Append("FROM SPC_SDPart_Tx_Power_XMR a ");
            cmdText.Append("WHERE 1=1 ");
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxPowerXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxPowerXMRInfo item = new SPCSDPartTxPowerXMRInfo();
        ...
        public SPCSDPartTxPowerXMRInfo Get(int XMRPK)
        {
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            ...
            DetailCmdText.Append("FROM SPC_SDPart_Tx_Power_Detail ");
            DetailCmdText.Append("WHERE XMR_PK=@XMR_PK ");
            DetailCmdText.Append("ORDER BY No_Of_Times ASC ");

            IList<SPCSDPartTxPowerDetail> DetailItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxPowerDetail>(CommandType.Text, DetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxPowerDetail item = new SPCSDPartTxPowerDetail();
            ...
            ExceptionCmdText.Append("FROM SPC_SDPart_Tx_Power_Exception ");
            ExceptionCmdText.Append("WHERE XMR_PK=@XMR_PK ");

            IList<SPCSDPartTxPowerException> ExceptionItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxPowerException>(CommandType.Text, ExceptionCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxPowerException item = new SPCSDPartTxPowerException();
                item.NoOfTimes = Convert.ToInt32(reader["No_Of_Times"]);
                item.TestingDate = Convert.ToDateTime(reader["Testing_Date"]);
            ...
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append("SELECT XMR_PK,Station_No,Divide,CH_No,Mode,CH,PW,Serial_No,Date_From,Date_To,X,R,CL_X,UCL_X,LCL_X,CL_R,UCL_R,LCL_R ");
            cmdText.Append("FROM SPC_SDPart_Tx_Power_XMR ");
            cmdText.Append("WHERE XMR_PK=@XMR_PK ");

            return AdoTemplate.QueryForObjectDelegate<SPCSDPartTxPowerXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxPowerXMRInfo entity = new SPCSDPartTxPowerXMRInfo();
        ...
        public void SaveException(int XMRPK, IList<SPCSDPartTxPowerException> ExceptionItems)
        {
            StringBuilder cmdText = new StringBuilder();
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            ...
            {
                cmdText.Append(" INSERT INTO SPC_SDPart_Tx_Power_Exception");
