...
{
    public class NTTxMaskFlat : AdoDaoSupport, INTTxMaskFlat
    {
        public int Query(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT distinct count(*) ");
            cmdText.Append(" FROM  nt_tx_maskflat_list a left join barcode_list b on a.serial_no=b.serial_no ");
        ...
        public IList<NTTxMaskFlatInfo> Query(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT * FROM (");

            cmdText.Append(" SELECT rowindex = row_number() over (order by " + sortBy + " " + orderBy + " ) ,");
            cmdText.Append(" a.nt_tx_maskflat_id,c.model,a.serial_no,a.station_no,a.ch_no,a.wg_no, ");
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<NTTxMaskFlatInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskFlatInfo item = new NTTxMaskFlatInfo();
                item.NTTxMaskFlatId = Convert.ToInt32(reader["nt_tx_maskflat_id"]);
        ...
        public IList<NTTxMaskFlatInfo> Query(Hashtable hashTable, string sortBy, string orderBy)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT distinct a.nt_tx_maskflat_id,c.model,a.serial_no,a.station_no,a.ch_no,a.wg_no, ");
            cmdText.Append(" a.start_time,a.end_time,a.app_version,final_flag,a.reason");
            cmdText.Append(" FROM  nt_tx_maskflat_list a left join barcode_list b on a.serial_no=b.serial_no ");
             cmdText.Append(" left join label_code c on b.code=c.code and b.model=c.model"); 
        ...
        public NTTxMaskFlatInfo GetDetail(int NTTxMaskFlatId)
        {
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            paras.Create().Name("nt_tx_maskflat_id").Type(DbType.Int32).Size(4).Value(NTTxMaskFlatId);

            StringBuilder NTTxMaskFlatDetailCmdText = new StringBuilder();
            NTTxMaskFlatDetailCmdText.Append(" SELECT mode,ch,mask_flat ");
            NTTxMaskFlatDetailCmdText.Append(" FROM nt_tx_maskflat_detail ");
            NTTxMaskFlatDetailCmdText.Append(" WHERE nt_tx_maskflat_id=@nt_tx_maskflat_id");
            NTTxMaskFlatDetailCmdText.Append(" ORDER BY mode");

            IList<NTTxMaskFlatDetailInfo> NTTxMaskFlatDetailItems = AdoTemplate.QueryWithRowMapperDelegate<NTTxMaskFlatDetailInfo>(CommandType.Text, NTTxMaskFlatDetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskFlatDetailInfo item = new NTTxMaskFlatDetailInfo();
            ...
            cmdText.Append(" WHERE 1=1 ");
            cmdText.Append(" AND a.nt_tx_maskflat_id=@nt_tx_maskflat_id");
            return AdoTemplate.QueryForObjectDelegate<NTTxMaskFlatInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskFlatInfo entity = new NTTxMaskFlatInfo();
                ...
                entity.NTTxMaskFlatDetailItems = NTTxMaskFlatDetailItems;
                return entity;
            }, paras.GetParameters());
        }
    }
