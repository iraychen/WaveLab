...
{
    public class SPCSDPartTxGainXMR : AdoDaoSupport, ISPCSDPartTxGainXMR
    {
        public int QueryHistory(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT DISTINCT COUNT(*) ");
            cmdText.Append(" FROM SPC_SDPart_Tx_Gain_XMR ");
        ...
        public IList<SPCSDPartTxGainXMRInfo> QueryHistory(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxGainXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxGainXMRInfo item = new SPCSDPartTxGainXMRInfo();
        ...
        public SPCSDPartTxGainXMRInfo Get(int XMRPK)
        {
            ...
            DetailCmdText.Append("FROM SPC_SDPart_Tx_Gain_Detail ");
            DetailCmdText.Append("WHERE XMR_PK=@XMR_PK ");
            DetailCmdText.Append("ORDER BY No_Of_Times ASC ");

            IList<SPCSDPartTxGainDetail> DetailItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxGainDetail>(CommandType.Text, DetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxGainDetail item = new SPCSDPartTxGainDetail();
            ...
            ExceptionCmdText.Append("FROM SPC_SDPart_Tx_Gain_Exception ");
            ExceptionCmdText.Append("WHERE XMR_PK=@XMR_PK ");

            IList<SPCSDPartTxGainException> ExceptionItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartTxGainException>(CommandType.Text, ExceptionCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxGainException item = new SPCSDPartTxGainException();
                item.NoOfTimes = Convert.ToInt32(reader["No_Of_Times"]);
                item.TestingDate = Convert.ToDateTime(reader["Testing_Date"]);
            ...
            return AdoTemplate.QueryForObjectDelegate<SPCSDPartTxGainXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartTxGainXMRInfo entity = new SPCSDPartTxGainXMRInfo();
        ...
        public void SaveException(int XMRPK, IList<SPCSDPartTxGainException> ExceptionItems)
        {
            StringBuilder cmdText = new StringBuilder();
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            ...
            {
                cmdText.Append(" INSERT INTO SPC_SDPart_Tx_Gain_Exception");
