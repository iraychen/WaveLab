...
{
    public class SPCSDPartRxPowerXMR : AdoDaoSupport, ISPCSDPartRxPowerXMR
    {
        public int QueryHistory(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT DISTINCT COUNT(*) ");
            cmdText.Append(" FROM SPC_SDPart_Rx_Power_XMR a ");
        ...
        public IList<SPCSDPartRxPowerXMRInfo> QueryHistory(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxPowerXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxPowerXMRInfo item = new SPCSDPartRxPowerXMRInfo();
        ...
        public SPCSDPartRxPowerXMRInfo Get(int XMRPK)
        {
            ...
            DetailCmdText.Append("FROM SPC_SDPart_Rx_Power_Detail ");
            DetailCmdText.Append("WHERE XMR_PK=@XMR_PK ");
            DetailCmdText.Append("ORDER BY No_Of_Times ASC ");

            IList<SPCSDPartRxPowerDetail> DetailItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxPowerDetail>(CommandType.Text, DetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxPowerDetail item = new SPCSDPartRxPowerDetail();
            ...
            ExceptionCmdText.Append("FROM SPC_SDPart_Rx_Power_Exception ");
            ExceptionCmdText.Append("WHERE XMR_PK=@XMR_PK ");

            IList<SPCSDPartRxPowerException> ExceptionItems = AdoTemplate.QueryWithRowMapperDelegate<SPCSDPartRxPowerException>(CommandType.Text, ExceptionCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxPowerException item = new SPCSDPartRxPowerException();
                item.NoOfTimes = Convert.ToInt32(reader["No_Of_Times"]);
                item.TestingDate = Convert.ToDateTime(reader["Testing_Date"]);
            ...
            cmdText.Append("FROM SPC_SDPart_Rx_Power_XMR ");
            cmdText.Append("WHERE XMR_PK=@XMR_PK ");

            return AdoTemplate.QueryForObjectDelegate<SPCSDPartRxPowerXMRInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                SPCSDPartRxPowerXMRInfo entity = new SPCSDPartRxPowerXMRInfo();
        ...
        public void SaveException(int XMRPK, IList<SPCSDPartRxPowerException> ExceptionItems)
        {
            StringBuilder cmdText = new StringBuilder();
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            ...
            {
                cmdText.Append(" INSERT INTO SPC_SDPart_Rx_Power_Exception");
