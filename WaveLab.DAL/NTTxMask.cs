...
{
    public class NTTxMask : AdoDaoSupport, INTTxMask
    {
        public int Query(Hashtable hashTable)
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT distinct count(*) ");
            cmdText.Append(" FROM  nt_tx_mask_list a left join barcode_list b on a.serial_no=b.serial_no ");
        ...
        public IList<NTTxMaskInfo> Query(Hashtable hashTable, string sortBy, string orderBy, int page, int pageSize)
        {
            ...
            cmdText.Append(" a.nt_tx_mask_id,c.model,a.serial_no,a.station_no,a.ch_no,a.wg_no, ");
            ...
            return AdoTemplate.QueryWithRowMapperDelegate<NTTxMaskInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskInfo item = new NTTxMaskInfo();
                item.NTTxMaskId = Convert.ToInt32(reader["nt_tx_mask_id"]);
        ...
        {
            StringBuilder cmdText = new StringBuilder();
            cmdText.Append(" SELECT distinct a.nt_tx_mask_id,c.model,a.serial_no,a.station_no,a.ch_no,a.wg_no, ");
            cmdText.Append(" a.start_time,a.end_time,a.app_version,final_flag,a.reason");
            cmdText.Append(" FROM  nt_tx_mask_list a left join barcode_list b on a.serial_no=b.serial_no ");
             cmdText.Append(" left join label_code c on b.code=c.code and b.model=c.model"); 
        ...
        {
            IDbParametersBuilder paras = base.CreateDbParametersBuilder();
            paras.Create().Name("nt_tx_mask_id").Type(DbType.Int32).Size(4).Value(NTTxMaskId);

            StringBuilder NTTxMaskDetailCmdText = new StringBuilder();
            NTTxMaskDetailCmdText.Append(" SELECT nt_tx_mask_id,mode,ch,mask_check,mask_image ");
            NTTxMaskDetailCmdText.Append(" FROM nt_tx_mask_detail");
            NTTxMaskDetailCmdText.Append(" WHERE nt_tx_mask_id=@nt_tx_mask_id");
            NTTxMaskDetailCmdText.Append(" ORDER BY mode");

            IList<NTTxMaskDetailInfo> NTTxMaskDetailItems = AdoTemplate.QueryWithRowMapperDelegate<NTTxMaskDetailInfo>(CommandType.Text, NTTxMaskDetailCmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskDetailInfo item = new NTTxMaskDetailInfo();
                item.NTTxMaskId = Convert.ToInt32(reader["nt_tx_mask_id"]);
            ...
            cmdText.Append(" WHERE 1=1 ");
            cmdText.Append(" AND a.nt_tx_mask_id=@nt_tx_mask_id");
            return AdoTemplate.QueryForObjectDelegate<NTTxMaskInfo>(CommandType.Text, cmdText.ToString(), delegate(IDataReader reader, int row)
            {
                NTTxMaskInfo entity = new NTTxMaskInfo();
                ...
                entity.NTTxMaskDetailItems = NTTxMaskDetailItems;
                return entity;
            }, paras.GetParameters());
        }
    }
